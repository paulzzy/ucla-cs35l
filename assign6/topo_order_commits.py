...
If the only line containing "execve" indicates that `pytest` was called, then
no other commands were called and my implementation is spec-compliant.
"""
...
class CommitNode:
    ...
    def object_path(self) -> str:
        """
        Assumes the current working directory is .git, returns the relative
        path of the object file associated with this commit.
        """
...
class GitRepo:
    ...
    def __build_commit_graph(self) -> None:
        ...
        def get_head_commits(branches: list[str]) -> list[CommitNode]:
            ...
            for branch in branches:
                ...
                # Branch name can have "/", causing its commit hash file inside
                # .git to be inside nested directories. For example, for branch
                # name a/b/c the commit hash file is located at
                # .git/refs/heads/a/b/c.
                #
                # Keep traversing down until a file is encountered.
                while not os.path.isfile(maybe_head_location):
                    maybe_head_location = (
                        f"{maybe_head_location}/"
                        f"{os.listdir(maybe_head_location)[0]}"
                    )
        ...
        def populate_tree(heads: list[CommitNode]) -> list[CommitNode]:
            """
            Uses multiple depth-first searches, each starting from a branch
            HEAD commit. Returns a list of root commits (those with no
            parents).
            """

            commits: list[CommitNode] = []
            existing: dict[str, CommitNode] = {
                head.commit_hash: head for head in heads
            }
            ...
            while dfs_stack:
                ...
                try:
                    ...
                    with open(
                        ...
                        text = (
                            zlib.decompress(commit_object.read())
                            .decode()
                            .split()
                        )
                        ...
                        for i, token in enumerate(text):
                            ...
                            if token == "author":
                                ...
                                # message. Since the message could contain
                                # "parents" on one line, which would break
                                # parsing, it's best to exit early.
                                break
                    ...
                    print(
                        ...
                        f"This repository may be using packfiles, "
    ...
    def __str__(self) -> str:
        ...
        for commit in self.topo_sorted_commits:
            ...
            # When current commit is not a parent of the previous, add a
            # "sticky end" and "sticky start" to indicate how to reconstruct
            # the commit graph. The sticky parts include information on the
            # previous commit's parents and the current commit's children.
            #
            ...
            if prev and prev not in commit.children:
                prev_parents: list[str] = [
                    parent.commit_hash for parent in prev.parents
                ]
                sticky_end: str = f"{' '.join(prev_parents)}="
                curr_children: list[str] = [
                    child.commit_hash for child in commit.children
                ]
                sticky_start: str = f"\n\n=" f"{' '.join(curr_children)}\n"
                current_line = f"{sticky_end}{sticky_start}{current_line}"

            if commit.commit_hash in self.branch_heads:
                branch_names = sorted(
                    self.branch_heads[commit.commit_hash],
                    key=lambda branch: branch,
                )
